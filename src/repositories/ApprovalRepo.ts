...
import { ApprovalRule, ApprovalRuleApprover, ApprovalRuleWithApprovers, ApprovalRequest, ApprovalRequestStatus, ApprovalRequestWithDetails } from '../types';
...
export const ApprovalRuleRepo = {
  async create(data: {
    ...
    const result = await db.runAsync(
      `INSERT INTO approval_rules (company_id, user_id, description, manager_id, manager_is_approver, sequential, min_approval_percentage)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [data.company_id, data.user_id, data.description, data.manager_id,
       data.manager_is_approver ? 1 : 0, data.sequential ? 1 : 0, data.min_approval_percentage]
    );
  ...
  async findByCompany(company_id: number): Promise<(ApprovalRule & { user_name: string })[]> {
    ...
    return db.getAllAsync(
      `SELECT ar.*, u.name as user_name FROM approval_rules ar
